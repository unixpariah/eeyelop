...
int compile_shader(const char *shader_source, int shader, int shader_program) {
...
EglSurface egl_surface_init(Egl *egl, struct wl_surface *surface, int size[2]) {
  EglSurface egl_surface = {
      .window = wl_egl_window_create(surface, size[0], size[1]),
  ...
  if (egl_surface.window == NULL || egl_surface.surface == NULL) {
    fprintf(stderr, "Failed to create egl window or surface");
    exit(1);
  }
